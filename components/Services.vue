<script lang="ts" setup>
</script>
<template>
  <section id="services" class="py-12 bg-gray-100">
    <div class="container mx-auto text-center">
      <h2 class="text-3xl font-bold mb-6">Our Services</h2>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div class="p-6 bg-white rounded-lg shadow-md">
          <!-- Phone Repairs Icon -->
          <Icon name="bi:phone" class="text-4xl text-yellow-500 mb-4" />
          <h3 class="text-xl font-semibold">Phone Repairs</h3>
          <p>All major brands</p>
        </div>
        <div class="p-6 bg-white rounded-lg shadow-md">
          <!-- Gadget Repairs Icon -->
          <Icon name="bi:laptop" class="text-4xl text-yellow-500 mb-4" />
          <h3 class="text-xl font-semibold">Gadget Repairs</h3>
          <p>Laptops, TVs, Computers</p>
        </div>
        <div class="p-6 bg-white rounded-lg shadow-md">
          <!-- Phones on Credit Icon -->
          <Icon name="bi:credit-card" class="text-4xl text-yellow-500 mb-4" />
          <h3 class="text-xl font-semibold">Phones on Credit</h3>
          <p>Get phones on affordable credit terms</p>
        </div>
      </div>
    </div>
  </section>
</template>


<style>
</style>
